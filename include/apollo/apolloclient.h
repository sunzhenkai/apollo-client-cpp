 ...
 */
#define APOLLO_CONFIG_NOTI_URL                                                 \
  "http://%s/notifications/v2?appId=%s&cluster=%s&notifications=%s"
 ...
 *     如:[{"namespaceName": "application", "notificationId":
 -1}],注意使用url转码
    2.
 服务端针对传过来的每一个namespace和对应的notificationId，检查notificationId是否是最新的
       如果是-1，返回初始化的notificationId。
       如果是最新的（传入的notificationId和服务保持一致），则保持住请求60秒，如果60秒内没有配置变化，则返回HttpStatus
 304（此时不会返回任何数据只有状态码）。
          如果60秒内有配置变化，则返回对应namespace的最新notificationId,
 返回HttpStatus 200。
       如果传过来的notifications信息中发现有notificationId比服务端老，则直接返回对应namespace的最新notificationId,
 HttpStatus 200。
    3. 客户端拿到服务端返回后，判断返回的HttpStatus
      如果返回的HttpStatus是304，说明配置没有变化，重新执行第1步
      如果返回的HttpStauts是200，说明配置有变化，针对变化的namespace重新去服务端拉取配置，参见1.3
 通过不带缓存的Http接口从Apollo读取配置。同时更新notifications
 map中的notificationId。重新执行第1步。
 * @param apolloEnv
