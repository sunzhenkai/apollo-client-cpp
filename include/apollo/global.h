...
#define newString(len)                                                         \
  len != 0 ? (char *)malloc(sizeof(char) * len)                                \
           : (char *)malloc(sizeof(char) * DEFAULT_CHARSIZE);
...
size_t write_memory_callback(void *ptr, size_t size, size_t nmemb,
                             void *stream);
struct curl_slist *setCommonHeader(CURLcode *res, String tokenHeader);
void jsonStrToProperties(String properStr, String childNodeName,
