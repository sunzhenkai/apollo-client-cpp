...
String getApolloConfig(ApolloConf apolloConf, String envName, String appId,
  ...
  if (curl) {
    ...
    String urlDest = sprintfStr(LATESTREKEASE_URL, apolloConf.config_server_url,
                                envName, appId, clusterName, namespaceName);
...
void getApolloClusterEnv(ApolloConf apolloConf, String appId, envclusters *ec) {
  ...
  if (curl) {
    ...
    strcpy(str, "");
    String urlDest =
        sprintfStr(CLUSTERENV_URL, apolloConf.config_server_url, appId);
    String tokenHeader = sprintfStr(TOKEN_PATTERN, apolloConf.token);
    struct curl_slist *headers = setCommonHeader(&res, tokenHeader);
    curl_easy_setopt(curl, CURLOPT_URL, urlDest);
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, write_memory_callback);
...
void getProperties(ApolloConf apolloConf, String envName, String appId,
  ...
  String str =
      getApolloConfig(apolloConf, envName, appId, clusterName, namespaceName);
  json_object *jsonObject = json_tokener_parse(str);
...
void getYamlOrJson(ApolloConf apolloConf, String envName, String appId,
  ...
  Properties *properties;
  getProperties(apolloConf, envName, appId, clusterName, namespaceName,
                properties);
