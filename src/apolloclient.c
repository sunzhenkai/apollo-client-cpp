...
String getNoCachePropertyString(apollo_env apolloEnv) {
  ...
  if (curl) {
    ...
    String urlDest =
        sprintfStr(APOLLO_CONFIG_NOCACHE_URL, apolloEnv.meta, apolloEnv.appId,
                   apolloEnv.clusterName, apolloEnv.namespaceName);
...
CURLcode checkNotifications(apollo_env apolloEnv, notification notifications,
                            long *response_code, String notificationReturn) {
  CURL *curl = curl_easy_init();
  char *buf = malloc(200);
  if (curl) {
    CURLcode res;
    ...
    String urlDest =
        sprintfStr(APOLLO_CONFIG_NOTI_URL, apolloEnv.meta, apolloEnv.appId,
                   apolloEnv.clusterName, notification);
    struct curl_slist *headers = setCommonHeader(&res, NULL);
    curl_easy_setopt(curl, CURLOPT_URL, urlDest);
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, write_memory_callback);
...
void submitNotifications(apollo_env apolloEnv, notification notifications,
                         int *flag,
                         void (*callback)(Properties *old, Properties *to)) {
  // 第一次获取资源信息。
  ...
  CURLcode res =
      checkNotifications(apolloEnv, notifications, &responseCode, notiStr);
  // 根据动态flag决定是否需要继续long pooling
  while (flag > 0) {
    if (res == CURLE_OK) {
      // 304直接使用当前数据直接调用当前方法继续递归
      if (res == 304) {
        res = checkNotifications(apolloEnv, notifications, &responseCode,
                                 notiStr);
        continue;
      } else {
        // 200不管是否都需要获取最新的配置项
        if (responseCode == 200) {
          ...
          json_object_object_get_ex(notifyObj, "notificationId",
